...
Handshake.prototype = extend(IHandshake.prototype, {
    ...
    handshake: function (client, onSuccess, onFail) {
        ...
        this.onSuccess = onSuccess;//SparkCore.ready function
        this.onFail = onFail;// SparkCore.disconnect function
    ...
    _nextStep: function (data) {
        switch (this.stage) {
            case Handshake.stages.SEND_NONCE:
                //send initial nonce unencrypted to the device
                this.send_nonce();
                break;

            case Handshake.stages.READ_COREID:
                //device send encrypted nonce, reads back our encrypted nonce and the coreid
            ...
            case Handshake.stages.SEND_SESSIONKEY:
                ...
                // From this step onwards we have session key as Symmetric key.
...
//
    ...
    read_coreid: function (data) {
        ...
        //The data contains the Nonce bytes that we sent and the device ID.
        //Thus, it should be 52 bytes.
        // Option:  You can send the Device's public key along with the Nonce and deviceID.
        ...
        if (plaintext.length > (Handshake.NONCE_BYTES + Handshake.ID_BYTES)) {
            ...
            // Option:  You can send the Device's public key along with the Nonce and deviceID.
            // This device public key is stored as "_handshake.pub.pem", which later can be verified
            ...
            try {
                if (!public_key) {
                    ...
                    if (that.coreProvidedPem) {
                        // Write the device's public key which was sent with the nonce as "_handshake.pub.pem"
...
//  * Server sends 384 bytes to Core: the ciphertext then the signature.
    ...
    //creates a session key, encrypts it using the core's public key, and sends it back
    // this also create  pipe for input and output, socket-->chunking streams-->secure streams.
    ...
    send_sessionkey: function () {
        ...
        CryptoLib.getRandomBytes(Handshake.SESSION_BYTES, function (ex, buf) {
            ...
            //Here the server sends the Session Key to the device
            that.socket.write(msg);
            //logger.log('Handshake: sent encrypted sessionKey');


            //Creates an cryptostreams for both incoming and outgoing data to the device.
            // uses AES, IV is first few bytes that of the previous stream.
            ...
            if (that.useChunkingStream) {
                ...
                // This creates pipe b/w the socket pipe and chunk pipe (which is required to parse data for packetization)
                that.socket.pipe(that.chunkingIn);
                // This creates a pipe between chunk and secureIn to decrypt the data.
                that.chunkingIn.pipe(that.secureIn);

                //what I send goes into the encrypter, and then gets broken into message chunks
                //Similar to above, encrypt-->chunkingpipe-->socket.
            ...
            // After the data on the secureIn pipeline is decrypted and is readable, routeToClient
    ...
    get_hello: function (data) {
        ...
        if (!data) {
            if (this._socketTimeout) {
                clearTimeout(this._socketTimeout);  //just in case, when the device takes longer time to send the hello packet
            }
