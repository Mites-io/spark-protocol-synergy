...
//    },
    ...
    getServerKeys: function () {
        if (!CryptoLib._serverKeys) {
            CryptoLib._serverKeys = crypto.generateKeyPairSync('rsa', {
                modulusLength: 2048,
                publicKeyEncoding: {
                    type: 'spki',
                    format: 'pem'
...
//    })(),
    ...
    getCoreKeys: function () {
        if (!CryptoLib._coreKeys) {
            CryptoLib._coreKeys = crypto.generateKeyPairSync('rsa', {
                modulusLength: 2048,
                publicKeyEncoding: {
                    type: 'spki',
                    format: 'pem'
...
//    })(),
    ...
    loadServerPublicKey: function(filename) {
        return utilities.promiseDoFile(filename,function (data) {
            ...
            CryptoLib.setServerKeys(createKey(data));
    ...
    loadServerKeys: function (filename, passFile, envVar) {
        ...
        var keys = crypto.createPrivateKey({
            key: data,
            format: 'pem',
            type: 'pkcs8',
            passphrase: password
        });
        // console.log(data.toString());  // Look at the format - if it starts with "-----BEGIN" it's PEM
...
function isKey(key) {
    try {
        return key.asymmetricKeyType === 'rsa' && 
               (key.type === 'private' || key.type === 'public');
    } catch (e) {
        return false;
    }
}
